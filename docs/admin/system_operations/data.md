...
|---    |---          |
|**Compaction Commands**||
|leofs-adm compact-start \<storage-node\> (all\|\<num-of-targets\>) [\<num-of-compaction-proc\>]|Start Compaction (Transfer its state to **running**).<p>num-of-targets: How many AVS/KVS pairs are compacted.<p>num-of-compaction-pro: How many processes are run in parallel. |
...
```
The file is formatted as Tab Separated Values(TSV) except headers (head 3 lines of a file).
...
## Recover Objects
This section provides information about recover commands that can be used in order to recover inconsistencies in a LeoFS cluster according to failures.
